...
async function start() {
  ...
  app.get('/api/production', (req, res) => {
    ...
    let query = `SELECT pl.*, e.name as employee_name, p.part_number, p.target_rate, c.name as customer_name, c.id as customer_id
  ...
  app.post('/api/production', (req, res) => {
    ...
    const insert = db.transaction(() => {
      const result = db.prepare(`INSERT INTO production_logs (date, shift, employee_id, part_id, qty_produced, qty_scrap, hours_run, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
  ...
  app.get('/api/reports/summary', (req, res) => {
    ...
    const byPart = db.prepare(`
      SELECT c.name as customer, p.part_number, p.target_rate,
             SUM(pl.qty_produced) as total_produced, SUM(pl.qty_scrap) as total_scrap,
             SUM(pl.hours_run) as total_hours, COUNT(pl.id) as run_count
      FROM production_logs pl
      JOIN parts p ON pl.part_id = p.id
      JOIN customers c ON p.customer_id = c.id
    ...
    const byEmployee = db.prepare(`
      ...
      FROM production_logs pl JOIN employees e ON pl.employee_id = e.id
      WHERE pl.date BETWEEN ? AND ? GROUP BY e.id ORDER BY total_produced DESC
    `).all(from, to);
    const byScrap = db.prepare(`
      ...
      WHERE pl.date BETWEEN ? AND ? GROUP BY sr.id ORDER BY total DESC
    `).all(from, to);
