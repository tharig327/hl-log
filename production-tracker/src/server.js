...
app.post('/api/customers/bulk', (req, res) => {
  ...
  const insert = db.transaction(() => {
    ...
    names.forEach(name => {
      ...
      if (!trimmed) return;
      try {
        db.prepare('INSERT INTO customers (name) VALUES (?)').run(trimmed);
        added++;
      } catch { skipped++; }
    });
...
app.get('/api/parts', (req, res) => {
  ...
  let query = `
    ...
    FROM parts p JOIN customers c ON p.customer_id = c.id
  `;
  const params = [];
...
app.post('/api/employees/bulk', (req, res) => {
  ...
  const insert = db.transaction(() => {
    ...
    employees.forEach(({ name, employee_id }) => {
      ...
      try {
        db.prepare('INSERT INTO employees (name, employee_id) VALUES (?, ?)').run(name.trim(), employee_id || null);
        added++;
      } catch { skipped++; }
    });
    return { added, skipped };
...
app.get('/api/production', (req, res) => {
  ...
  logs.forEach(log => {
    ...
    log.actual_rate = (log.hours_run && log.hours_run > 0)
      ? Math.round(log.qty_produced / log.hours_run)
      : null;
    log.efficiency = (log.actual_rate && log.target_rate)
      ? Math.round((log.actual_rate / log.target_rate) * 100)
      : null;
  });
  res.json(logs);
});
...
app.get('/api/reports/summary', (req, res) => {
  ...
  byPart.forEach(row => {
    row.scrap_pct = row.total_produced > 0
      ? ((row.total_scrap / (row.total_produced + row.total_scrap)) * 100).toFixed(1)
      : '0.0';
    row.actual_rate = (row.total_hours > 0)
      ? Math.round(row.total_produced / row.total_hours)
      : null;
    row.efficiency = (row.actual_rate && row.target_rate)
      ? Math.round((row.actual_rate / row.target_rate) * 100)
      : null;
  });
