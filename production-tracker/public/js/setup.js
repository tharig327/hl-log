...
async function loadCustomers() {
  ...
  page.innerHTML = `
    ...
    <div class="card" style="margin-bottom:20px;border-left:4px solid var(--blue)">
      ...
      <div class="two-col">
        ...
        <div>
          ...
          <div style="font-size:11px;color:var(--gray);margin-top:4px">Columns: Customer Name, Part Number, Description (optional), Target Rate (optional). Customer is auto-created if it doesn't exist.</div>
    ...
    <div class="two-col">
      <div class="card">
        <div class="section-header"><h2>Add Single Customer</h2></div>
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Customer</th><th>Parts</th></tr></thead>
            <tbody>
              ${customers.map(c => {
                const pc = allParts.filter(p => p.customer_id === c.id).length;
                return `<tr><td>${c.name}</td><td><span class="badge badge-blue">${pc}</span></td></tr>`;
              }).join('') || '<tr><td colspan="2" style="text-align:center;color:#999;padding:24px">No customers yet</td></tr>'}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div class="card">
        <div class="section-header"><h2>Add Single Part</h2></div>
        ...
        <div style="margin-top:24px">
          ...
          <div class="table-wrap" style="margin-top:12px">
            <table>
              <thead><tr><th>Customer</th><th>Part #</th><th>Description</th><th>Target Rate</th></tr></thead>
              <tbody>
                ${allParts.map(p => `<tr>
                ...
                </tr>`).join('') || '<tr><td colspan="4" style="text-align:center;color:#999;padding:24px">No parts yet</td></tr>'}
              </tbody>
            </table>
          </div>
        </div>
  ...
  document.getElementById('bulk-parts-btn').addEventListener('click', async () => {
    ...
    const rows = lines.map(line => {
      ...
      return {
        ...
        target_rate:   cols[3] || null
      };
    });
    try {
      const res = await API.post('/api/parts/bulk', { rows });
      let msg = `&#10003; ${res.added} part(s) added`;
      if (res.skipped) msg += `, ${res.skipped} skipped (already exist or blank)`;
...
async function loadEmployees() {
  ...
  page.innerHTML = `
    ...
    <div class="card" style="margin-bottom:20px;border-left:4px solid var(--blue)">
      ...
      <h2 style="font-size:14px;color:var(--gray);text-transform:uppercase;letter-spacing:.5px">One employee per line &mdash; Name or Name, EmployeeID</h2>
      ...
      <div style="font-size:11px;color:var(--gray);margin-top:4px">Optional: add a comma and an employee/badge ID after the name.</div>
      <br/>
      <button class="btn btn-primary btn-sm" id="bulk-employees-btn">Import Employees</button>
          ...
          <table>
            ...
            <tbody>
              ${employees.map(e => `<tr>
                ...
                <td><button class="btn btn-ghost btn-sm" onclick="deactivateEmp(${e.id})">Deactivate</button></td>
